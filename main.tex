...
into flanking genic sequence \citep{huang2022species}.
...
This interpretation gains mechanistic specificity
from the isoform regression results.
KDM3A, the sole significant predictor of Z:A compensation ratios
($p = 0.008$), is an H3K9 demethylase---it removes
the exact repressive mark deposited by TE silencing.
Under this model, KDM3A is the enzymatic bottleneck
that determines how far the compensation program can reach
into LINE-dense chromatin:
its female isoform proportion sets the demethylation rate,
while LINE density sets the H3K9me deposition rate.
In LINE-sparse regions, KDM3A clears repressive marks
faster than they accumulate,
and compensation succeeds.
In LINE-dense regions---specifically the highest RTE-BovB quartile---the
H3K9me load exceeds KDM3A's capacity,
the chromatin remains closed,
and the gene escapes compensation.
The categorical nature of the quartile effect
(no consistent sex bias in Q1--Q3, 89\% male-biased in Q4)
is consistent with a saturation threshold:
KDM3A handles background H3K9me adequately up to a density threshold,
beyond which the system is overwhelmed.
CTCF insulators, which we found enriched on Z
and positioned in gaps between LINE elements,
may serve as firebreaks that prevent LINE-associated heterochromatin
from spreading into adjacent compensated domains,
thereby sharpening the boundary between compensated and escapee regions.

This reframing also connects the LINE landscape
to the gene-body cis-element results.
Compensated Z genes carry Motif A + Motif B composite elements
while escapees lack them (Fisher $p = 0.007$).
If LINE-dense heterochromatin renders these motifs
inaccessible to the trans-acting factors
that recognize them---whether FoxA2 or another pioneer---then
motif absence and LINE density are not independent predictors
of escapee status but two faces of the same chromatin barrier.
The motifs may be present in the underlying sequence
but functionally invisible when embedded
in H3K9me-marked heterochromatin.
