...
reuses familiar components---chromatin regulators, cis-elements, repeats, and lncRNAs---
but in a configuration that appears distinct from previously studied organisms.
\end{abstract}
...
in the female relative to the male--- further confirmation of the previously identified Z chromosome (Figure~\ref{fig:seq_depth}).
...
Namely, we compared the \illex assembly to published genomes of 
...
the Z chromosome has maintained its integrity as a distinct chromosomal unit for over 300 million years of evolution (Figure~\ref{fig:phylo}).
...
is that the hemizygous Z undergoes transcriptional upregulation.
\comment{This is the part where I  still struggle to fully understand the argument. The pattern is the same for mammals right? The ratio of X:AA and XX:AA are both near 1 https://www.nature.com/articles/ng.991. There we know that there's a two step process -- inactivation of one X in females and upregulation of the X in *both* sexes. Can we rule out the same thing here??
We don't really discuss the possibilty of Z inactivation in males, just upregulation in females.}
...
In every species examined, the same pattern was found, where Z-linked gene expression in females apprixamtely matches the level observed in males,
consistent with active dosage compensation of the hemizygous Z chromosome
...
Genome-wide, we observed a positive correlation between gene expression and methylation levels in both sexes (females: r=0.396, p=2.16$\times$10$^{-9}$; males: r=0.318, p=5.10$\times$10$^{-6}$).
...
Crucially, LINE density on the Z predicts which genes escape dosage compensation. \comment{so cool!}
...
89\% of sex-biased genes are male expression biased (8M:1F),
...
We used Find Individual Motif Occurrences (FIMO) scanning to search for CTCF binding motifs
...
\comment{point to the parts of the results/figure that support this statement. Does this in anyway help with the Z incativation versus only female upregulation problem I mention above?}
...
%Figure is missing so can't compile
%\begin{figure}[H]
%    \centering
%    \includegraphics[width=\linewidth]{figures/illex_chrom_seq_depth}
