...
\begin{abstract}
Chromosome-wide dosage compensation has evolved independently in multiple animal lineages,
but the underlying mechanisms have been characterized in only a few.
The recent discovery of a conserved Z chromosome across coleoid cephalopods
raises the question of whether and how these animals have evolved dosage compensation.
Here, using a new chromosome-level genome assembly for the northern shortfin squid
\textit{Illex illecebrosus}
and comparative transcriptomics across eight cephalopod species spanning four orders
and over 300~million years of divergence,
we show that coleoid cephalopods have independently evolved
near-complete Z chromosome dosage compensation (87--103\% equalization).
Evolution of this system predates the divergence of octopuses and decapods,
indicating that compensation is ancestral to all coleoids.
The cephalopod Z chromosome carries a $\sim$10-fold enrichment of chromatin-modifying genes
(16 of 188 conserved Z genes),
nearly all of which are themselves compensated---creating
a self-reinforcing feedback loop
in which epigenetic machinery maintains the permissive chromatin state
that sustains its own single-copy expression.
Sex-biased isoform usage of just four of these chromatin remodelers
explains 57\% of Z:A expression variance,
with the H3K9 demethylase \textit{KDM3A} as the sole significant predictor.
Contrary to Lyon's repeat hypothesis for the mammalian X,
LINE-dense regions on the Z are where genes \textit{escape} compensation,
consistent with TE-associated heterochromatin acting as a barrier to upregulation.
We identify gene-body cis-element motifs
that discriminate compensated from escaped Z genes,
functionally analogous to \textit{Drosophila} MSL Recognition Elements,
and two deeply conserved sex-specific long non-coding RNAs---\textit{Zmast}
(male-biased, 5,000:1) and \textit{Zfest} (female-biased)---whose
conserved RNA-binding protein sites suggest a parallel
splicing regulatory axis.
These results establish cephalopods as a new model
for the evolution of chromosome-wide dosage compensation,
with a mechanism that assembles familiar components---chromatin modifiers,
cis-elements, and regulatory lncRNAs---in a configuration seen nowhere else.
\end{abstract}
...
For example, in mammals, one X chromosome is transcriptionally silenced in females via the \textit{Xist} long non-coding RNA \citep{lyon1961gene, brockdorff2015dosage}; 
...
\citep{lyon1961gene},
\textit{Drosophila} \citep{lucchesi2015dosage},
\textit{C. elegans} \citep{meyer2005x}---and
birds, the best-studied ZZ/ZW system,
achieve only incomplete, gene-by-gene equalization
\citep{mank2009w}.
Here we show that cephalopods have independently evolved
near-complete dosage compensation (87--103\% equalization)
...
Z-enriched chromatin modifiers that maintain their own compensation
...
Our results generalize this finding across eight species,
four orders, and over 300~MY of divergence
(Table~\ref{tab:cross_species_dc}).
That compensation is present in every species examined---and
more complete than in birds---demonstrates
...
provides a direct quantitative link between epigenetic machinery
and chromosome-wide compensation output.
In \textit{O. bimaculoides}, sex-biased isoform proportions
of just four chromatin remodelers explain 57\% of Z:A variance
...
connecting the removal of repressive chromatin marks
...
\textit{KDM3A} provides the enzymatic link.
...
but the bulk TE landscape may simultaneously resist upregulation.
Whether this negative association between bulk TE density and
compensation holds on the \textit{Drosophila} X remains to be tested.
