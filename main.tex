...
We propose that this inversion reflects the local chromatin consequences
of TE silencing.
Host genomes repress TEs via repressive chromatin marks (e.g.\ H3K9me2/3),
and these marks spread into flanking genic sequence---an
effect that is prevalent and varies substantially across species
\citep{huang2022species}.
On the cephalopod Z, LINE-dense regions would thus be
locally heterochromatinized regardless of whether
the dosage compensation machinery reaches them.
On a single-copy Z in ZO females this local silencing
has a proportionally larger impact on transcriptional output
than in ZZ males, where a second copy buffers the effect,
producing the observed male-biased expression in LINE-rich regions.
The male bias need not reflect a failure of compensation per se,
but rather a chromatin environment that is refractory to
full transcriptional output from a single copy.
...
which must upregulate the single female copy
